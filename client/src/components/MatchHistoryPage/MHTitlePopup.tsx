...
const MHTitlePopup = () => {
  ...
  const handleFormSubmission = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const matchData = await handleMatchSearch(matchID, setLoading, setFetchErr, setErrMessage);
    if (matchData) {
    ...
    <>
      ...
      <div
        ...
        <div className="flex flex-col items-center md:w-1/2 xl:w-1/3 opacity-0 [animation-delay:500ms] animate-slide-in-left">
          <p className="summary text-lg md:text-xl">Search up your match history!</p>
          ...
          <p className="text-white/60 text-lg md:text-xl text-center">
            You can find the match ID on the top right in the post-game screen in the client.
          </p>
          <div className="flex flex-col">
            <form className="text-center" onSubmit={handleFormSubmission}>
              <input
                ...
                className="px-2 py-2 text-2xl bg-gray text-white rounded-md mt-2"></input>
              <button
            ...
            <p className={`text-red text-center ${fetchErr ? "" : "opacity-0"}`}>
              Error: {errMessage}
            </p>
          </div>
        </div>
